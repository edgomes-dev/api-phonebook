...
import { INewContact } from "../type/contactType";
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (telephoneExists) {
      return res
        .status(500)
        .json({ message: "Contato com esse número já existe" });
    }
    ...
    try {
      ...
      return res
        .status(201)
        .json({ message: "Contato inserido no sistema com sucesso" });
    } catch (error) {
...
/* ----------------------------- Homepage / Pegar todos os contatos que não esteja excluido -----------------*/
...
router.get("/:group", async (req, res) => {
  ...
  if (group === "job") {
    try {
      const contactGroup = await ContactModel.find({
        group: "job",
        deleted_at: null,
      });
      const mixed = await ContactModel.find({
        group: "mixed",
        deleted_at: null,
      });
    ...
    try {
      const contactFav = await ContactModel.find({
        isFavorite: true,
        deleted_at: null,
      });
